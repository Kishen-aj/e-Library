...
namespace e_Library.Services
{
    public class BasketService : IBasketService
    {
        ...
        IRepository<PreOrderBook> prebookContext;
        IRepository<Basket> basketContext;

        public const string BasketSessionName = "eCommerceBasket";

        public BasketService(IRepository<Book> BookContext, IRepository<PreOrderBook> PreBookContext, IRepository<Basket> BasketContext)
        {
            this.basketContext = BasketContext;
            this.bookContext = BookContext;
            this.prebookContext = PreBookContext;
        }
        ...
        public void AddToBasket(HttpContextBase httpContext, string bookId)
        {
            ...
            PreOrderBook BookToEdit = prebookContext.Find(bookId);
        ...
        public void AddPreOrderToBasket(HttpContextBase httpContext, string bookId)
        {
            Basket basket = GetBasket(httpContext, true);
            BasketItem item = basket.BasketItems.FirstOrDefault(i => i.BookId == bookId);
            ...
            prebookContext.Commit();

        }

        public void RemovePreOrderFromBasket(HttpContextBase httpContext, string itemId)
        {
            Basket basket = GetBasket(httpContext, true);
            BasketItem item = basket.BasketItems.FirstOrDefault(i => i.Id == itemId);
        ...
        public List<BasketItemViewModel> GetPreOrderBasketItems(HttpContextBase httpContext)
        {
            Basket basket = GetBasket(httpContext, false);
            ...
            {
                var results = (from b in basket.BasketItems
                               join p in prebookContext.Collection() on b.BookId equals p.Id
                               select new BasketItemViewModel()
                               {
                                   Id = b.Id,
                                   Quantity = b.Quantity,
        ...
        public BasketSummaryViewModel GetPreOrderBasketSummary(HttpContextBase httpContext)
        {
            Basket basket = GetBasket(httpContext, false);
            BasketSummaryViewModel model = new BasketSummaryViewModel(0, 0);
            if (basket != null)
            {
                ...
                decimal? basketTotal = (from item in basket.BasketItems
                                        join p in prebookContext.Collection() on item.BookId equals p.Id
                                        select item.Quantity * p.Price).Sum();
        ...
        public decimal PreOrderBasketTotal(HttpContextBase httpContext)
        {
            Basket basket = GetBasket(httpContext, false);
            decimal bTotal = 0;
