...
namespace e_Library.WebUI.Controllers
{
    ...
    public class PreOrderManagerController : Controller
    {
        ...
        {
            List<PreOrder> orders = orderService.GetPreOrderList();
            orders = orderService.GetPreOrderList().Where(p => (p.Driver == "No Driver Required" && (p.DeliveryMethod == "Normal Collection" || p.DeliveryMethod == "Delayed Collection") && (p.OrderStatus == "Order Ready"))).ToList();
        ...
        {
            List<PreOrder> orders = orderService.GetPreOrderList();
            orders = orderService.GetPreOrderList().Where(p => (p.OrderStatus == "Pending Collection" && (p.DeliveryMethod == "Normal Collection" || p.DeliveryMethod == "Delayed Collection"))).ToList();
        ...
        [HttpPost]
        public ActionResult PrepareCollection(PreOrder preparedOrder, string Id)
        {
            PreOrder order = orderService.GetPreOrder(Id);
